<!-- Imagen encabezado -->
<div class="d-flex justify-content-center align-items-center position-relative text-center text-white"
        style="height: 250px;">
        <div class="position-absolute top-0 start-0 w-100 h-100 bg-dark" style="opacity: 0.8;"></div>
        <img src="img/encabezadoservicios.png" class="img-fluid w-100 h-100" alt="Productos y servicios de corte láser"
            style="object-fit: cover; opacity: 1;">
        <div class="position-absolute top-50 start-50 translate-middle">
            <h2 class="fw-bold">Servicios de Corte Láser</h2>
        </div>
    </div>

    <div class="container py-4">
        <!-- Icon Section -->
        <div class="container">
            <div class="row text-center icon-section" style="justify-content: space-around;">
                <div class="col-md-2 col-sm-4 mb-3">
                    <img src="img/imagenideas.png" alt="Envío de Idea" class="img-fluid mb-2"
                        style="width: 200px; height: auto;">
                    <h5 class="fw-bold text-warning">Envías tu idea</h5>
                    <p>Mandanos una imagen, un bosquejo o tus vectores.</p>
                </div>
                <div class="col-md-2 col-sm-4 mb-3">
                    <img src="img/imagencotizamos.png" alt="Cotizamos" class="img-fluid mb-2"
                        style="width: 200px; height: auto;">
                    <h5 class="fw-bold  text-warning">Cotizamos</h5>
                    <p>Te enviamos el costo de tu proyecto.</p>
                </div>
                <div class="col-md-2 col-sm-4 mb-3">
                    <img src="img/imagenpagar.png" alt="Pagas" class="img-fluid mb-2"
                        style="width: 200px; height: auto;">
                    <h5 class="fw-bold  text-warning">Pagas</h5>
                    <p>Si el precio te conviene, haces el pago.</p>
                </div>
                <div class="col-md-2 col-sm-4 mb-3">
                    <img src="img/imagenfabricamos.png" alt="Fabricamos" class="img-fluid mb-2"
                        style="width: 200px; height: auto;">
                    <h5 class="fw-bold  text-warning">Fabricamos</h5>
                    <p>Procedemos al corte y fabricación de tu proyecto.</p>
                </div>
                <div class="col-md-2 col-sm-4 mb-3">
                    <img src="img/imagenenviamos.png" alt="Enviamos" class="img-fluid mb-2"
                        style="width: 200px; height: auto;">
                    <h5 class="text-warning">Enviamos</h5>
                    <p>Enviamos a todo el país.</p>
                </div>
            </div>
        </div>
        <!-- Personalización de Productos Section -->
        <div class="position-relative text-center">
            <div class="position-absolute top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-dark text-white"
                style="opacity: 0.9;">

                <div class="col-md-6 col-8" style="text-align: left;">
                    <h2>Personalización de Productos</h2>
                    <p>Hemos creado una gama de productos que están listos para ser personalizados. Ideales para
                        souvenirs empresariales y regalos conmemorativos de fiestas u ocasiones especiales.</p>
                    <a href="Tienda.html" class="btn btn-warning">Ver Productos</a>
                </div>
                <div class="col-md-2 col-2 d-flex align-items-center justify-content-center">
                    <img src="img/imagen1per.png" alt="" class="img-fluid">
                </div>
                <div class="col-md-2 col-2 d-flex align-items-center justify-content-center">
                    <img src="img/imagen2per.png" alt="" class="img-fluid">
                </div>
            </div>
            <img src="img/infoper.png" alt="" class="img-fluid w-100">
        </div>

        <!-- Cotizar Mi Proyecto Section -->
        <div class="row text-center" style="margin: 80px 0px 60px 0px;">
            <div class="col-12">
                <h2 class="section-title">Cotizar Mi Proyecto</h2>
            </div>
        </div>
        <div class="container my-5">
            <div class="row text-center">
                <div class="col-md-6 mb-4 d-flex">
                    <div class="col-4 d-flex justify-content-center align-items-center">
                        <img src="img/imagen1cotizar.png" alt="" class="img-fluid">
                    </div>
                    <div class="col-8 text-left">
                        <h5><a class="text-warning" style="text-decoration: none;" href="#">Corte de piezas a medida</a>
                        </h5>
                        <p>Ideal para ingenieros, estudiantes o aficionados que desean crear un proyecto. Cortamos
                            plywood, plywood y acrílicos en medidas desde 1 milímetro hasta 5 milímetros.</p>
                        <a href="Categoria1.html" class="btn btn-warning">Cotizar Cortes</a>
                    </div>
                </div>
                <div class="col-md-6 mb-4 d-flex">
                    <div class="col-4 d-flex justify-content-center align-items-center">
                        <img src="img/imagen2cotizar.png" alt="" class="img-fluid">
                    </div>
                    <div class="col-8 text-left">
                        <h5><a class="text-warning" style="text-decoration: none;" href="#">Cajas en MDF o Acrílico</a>
                        </h5>
                        <p>Creación de cajas según las medidas del cliente. Servicios de corte y armado de cajas en MDF
                            y acrílico en medidas desde 2.5 milímetros hasta 5 milímetros.</p>
                        <a href="Categoria2.html" class="btn btn-warning">Cotizar Cajas</a>
                    </div>
                </div>
                <div class="col-md-6 mb-4 d-flex">
                    <div class="col-4 d-flex justify-content-center align-items-center">
                        <img src="img/imagen3cotizar.png" alt="" class="img-fluid">
                    </div>
                    <div class="col-8 text-left">
                        <h5><a class="text-warning" style="text-decoration: none;" href="#">Grabado láser en superficies
                                cilíndricas</a></h5>
                        <p>Ideal para botellas, pachones, tazas o cualquier superficie cilíndrica que pueda ser
                            trabajada en un grabador láser.</p>
                        <a href="Categoria3.html" class="btn btn-warning">Cotizar Grabados</a>
                    </div>
                </div>
                <div class="col-md-6 mb-4 d-flex">
                    <div class="col-4 d-flex justify-content-center align-items-center">
                        <img src="img/imagen4cotizar.png" alt="" class="img-fluid">
                    </div>
                    <div class="col-8 text-left">
                        <h5><a href="#" class="text-warning" style="text-decoration: none;">Grabado láser en superficies
                                planas</a></h5>
                        <p>Grabados en cuero, madera, tablas, acrílicos y cualquier superficie plana que pueda ser
                            trabajada con un grabador láser.</p>
                        <a href="Categoria2.html" class="btn btn-warning">Cotizar Grabado Plano</a>
                    </div>
                </div>
            </div>
        </div>
    </div>